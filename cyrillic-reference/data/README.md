...
Machine-readable counterparts to the Markdown tables in the parent directory. Generated by [`../tools/generate_json.py`](../tools/generate_json.py) — pure Python 3 stdlib, reads `cyrillic-languages/` directly via the shared [`../tools/_catalog.py`](../tools/_catalog.py) core. Independent of `generate.py` and `generate_svgs.py`; the three generators are siblings that can run in any order.
...
| `decomposition` | Ordered array of hex codepoints this character decomposes to under the project's Paratype-aware NFD. Empty `[]` when the character is a structural modification (DESCENDER, HOOK, TAIL, STROKE, BAR, TICK, UPTURN, MIDDLE HOOK, VERTICAL STROKE, STROKE-AND-HOOK) with no standard combining-mark equivalent. The Paratype-shaped Cyrillic breve appears as `F6D1` (uppercase) / `F6D4` (lowercase). Lowercase `ї` (U+0457) keeps the canonical `0456 + 0308` pair; renderers that do not honour Unicode `Soft_Dotted` should substitute `U+0131` (`dotlessi`) for `U+0456` — see the parent README's [Soft_Dotted bases](../README.md#soft_dotted-bases-lowercase-%D1%97) section. |
